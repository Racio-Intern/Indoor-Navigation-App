...
JNIEXPORT jdoubleArray JNICALL
        Java_apriltag_OpenCVNative_find_1camera_1focal_1length(JNIEnv *env, jclass clazz, jdoubleArray tag_corner_arr,
...
vector<Point2f> image_point = {Point2f((float) tag_corner[0], (float) tag_corner[1]), Point2f((float) tag_corner[2], (float) tag_corner[3]),
                               Point2f((float) tag_corner[4], (float) tag_corner[5]), Point2f((float) tag_corner[6], (float) tag_corner[7])};

vector<Point3f> object_points = {Point3f(-SQUARE_LENGTH / 2, SQUARE_LENGTH / 2, 0), Point3f(SQUARE_LENGTH / 2, SQUARE_LENGTH / 2, 0),
                                 Point3f(SQUARE_LENGTH / 2, -SQUARE_LENGTH / 2, 0), Point3f(-SQUARE_LENGTH / 2, -SQUARE_LENGTH / 2, 0)};
obj_points.push_back(object_points);
img_points.push_back(image_point);

Mat cameraM;
cameraM = initCameraMatrix2D(obj_points, img_points, cv::Size(image_size_arr[0], image_size_arr[1]));
...
vector<Point3f> object_point = {Point3f(-SQUARE_LENGTH / 2, SQUARE_LENGTH / 2, 0), Point3f(SQUARE_LENGTH / 2, SQUARE_LENGTH / 2, 0),
                                Point3f(SQUARE_LENGTH / 2, -SQUARE_LENGTH / 2, 0), Point3f(-SQUARE_LENGTH / 2, -SQUARE_LENGTH / 2, 0)};
...
// 카메라 rotation과 translation 벡터 찾기
Mat rvecs, tvecs; // 카메라 rotation, translation
solvePnP(object_point, image_point, cameraM, distortionC, rvecs, tvecs, false, SOLVEPNP_IPPE_SQUARE);
...
jobject camera_pos_estimation = (*env).NewObject(state.camera_pos_esti_cls, state.camera_pos_esti_constructor);

jdoubleArray cpe_rvecs =  reinterpret_cast<jdoubleArray>((*env).GetObjectField(camera_pos_estimation, state.cpe_rvecs_field));
(*env).SetDoubleArrayRegion(cpe_rvecs, 0, 3, (double *)rvecs.data);

jdoubleArray cpe_tvecs = reinterpret_cast<jdoubleArray>((*env).GetObjectField(camera_pos_estimation, state.cpe_tvecs_field));
(*env).SetDoubleArrayRegion(cpe_tvecs, 0, 3, (double *)tvecs.data);

jdoubleArray cpe_pos = reinterpret_cast<jdoubleArray>((*env).GetObjectField(camera_pos_estimation, state.cpe_relative_camera_pos_field));
